...
/**
 * Native Integer LinkedHashMap This class is NOT Thread-Safe
 * 
 * @param <V>
 *            type of values
 * 
...
public class IntLinkedHashMap<V> implements Iterable<V> {
	...
	private OldValueRemoveListener<V> OldValueRemoveListener;
	...
	/**
	 * Constructs a new {@code IntLinkedHashMap} instance with the specified
	 * capacity.
	 * 
	 * @param capacity
	...
	public void setOldValueRemoveListener(OldValueRemoveListener<V> oldValueRemover) {
		this.OldValueRemoveListener = oldValueRemover;
	}
	...
	public V put(final int key, final V value) {
		...
		if (entry == null) {
			// Remove eldest entry if instructed, else grow capacity if
			// appropriate
			IntLinkedEntry<V> eldest = header.after;
			++elementCount;
			// System.out.println("elementCount:" + elementCount + " ---> " +
			// (this.elementCount > this.threshold));
			if (elementCount > threshold) {
				if (removeEldestEntry(eldest)) {
					V removeValue = remove(eldest.key);
					// System.out.println("try to remove:" + eldest.key +
					// "\tremoveValue:" + removeValue);
					if (this.OldValueRemoveListener != null && removeValue != null) {
						this.OldValueRemoveListener.onRemove(removeValue);
					}
				} else {
					rehash();
	...
	private IntLinkedEntry<V> reuseAfterDelete() {
		...
		// if (reuse != null) {
		// if (log.isDebugEnabled()) {
		// log.debug("reusing IntLinkedEntry<V>=" + reuse.hashCode() + "
