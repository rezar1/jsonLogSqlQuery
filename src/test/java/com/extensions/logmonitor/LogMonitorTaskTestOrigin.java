...
public class LogMonitorTaskTestOrigin {
	...
	// assertEquals(3,
	// result.getMetrics().get("active-dynamic-*|SearchString|Debug|Debug").intValue());
	// assertEquals(0,
	// result.getMetrics().get("active-dynamic-*|SearchString|Error|Error").intValue());
	// long filesize = getFileSize(log.getLogDirectory(), testFilename);
	// assertEquals(filesize, result.getMetrics().get("active-dynamic-*|Filesize
	// (Bytes)").intValue());
	// // simulate our filepointer was updated
	// filePointer.updateLastReadPosition(filesize);
	// when(mockFilePointerProcessor.getFilePointer(anyString(),
